...
- Paper overview generation now follows the resolved PaperQuay UI language more strictly. When the interface is Chinese, generated abstracts, overviews, findings, takeaways, and keywords are prompted to be Chinese as well.
- Updated the overview prompt version so cached results from the older language-agnostic prompt are not reused for newly generated summaries.
...
- Continued the automated multi-platform release packaging flow for Windows, macOS Intel, macOS Apple Silicon, and Linux, with Windows release assets including both `.exe` and `.msi` packages.

## Included In This Release

- Strengthened the AI paper overview prompt so all JSON string values and arrays use the selected output language.
- Kept overview language resolution centralized, including the default follow-UI behavior and custom language overrides.
...
- Added Windows MSI packaging to the automated release assets alongside the NSIS `.exe` installer.
...
- 论文概览生成现在会更严格地跟随 PaperQuay 的界面语言。界面为中文时，生成的摘要、概览、发现、结论、要点和关键词都会被要求使用中文。
- 更新概览 prompt 版本，避免继续复用旧的、不带明确语言约束的缓存结果。
...
- 继续使用自动化多平台发布流程，覆盖 Windows、macOS Intel、macOS Apple Silicon 和 Linux，并让 Windows 发布资产同时包含 `.exe` 和 `.msi` 安装包。

## 本次版本包含

- 强化 AI 论文概览 prompt，要求所有 JSON 字符串字段和数组条目使用选定输出语言。
- 统一概览语言解析逻辑，保留默认跟随界面语言和自定义语言覆盖。
...
- 自动发布资产中新增 Windows MSI 安装包，同时保留 NSIS `.exe` 安装包。
